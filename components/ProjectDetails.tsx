...
export default function ProjectDetails({project}:{project:Film}) {
 ...
 <Dialog open={photo!==null} onOpenChange={open=>{if(!open)setPhoto(null)}}><DialogContent className="photo-viewer" showCloseButton={false} finalFocus={photoTrigger}><div className="photo-toolbar"><DialogTitle className="photo-title">{currentPhoto?.caption||project.title}</DialogTitle><DialogClose className="photo-back"><ArrowLeft className="ui-icon" aria-hidden="true"/> Back to project</DialogClose></div><DialogDescription className="sr-only">Enlarged project image. Close to return to the project.</DialogDescription>{currentPhoto&&<img className="photo-full" src={currentPhoto.image} alt={currentPhoto.caption||project.title}/>}<div className="photo-controls"><button disabled={photo===0} onClick={()=>setPhoto(i=>Math.max(0,(i??0)-1))}><ArrowLeft className="ui-icon" aria-hidden="true"/> Previous</button><span>{(photo??0)+1} / {images.length}</span><button disabled={photo===images.length-1} onClick={()=>setPhoto(i=>Math.min(images.length-1,(i??0)+1))}>Next <ArrowRight className="ui-icon" aria-hidden="true"/></button></div></DialogContent></Dialog>
 </>;
}
